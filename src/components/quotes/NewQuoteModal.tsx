...
export default function NewQuoteModal({
  ...
  useEffect(() => {
    if (editQuote) {
      ...
      setQuoteData({
        ...
        deliveryTerms: editQuote.deliveryTerms || "",
        warranty: editQuote.warranty || "",
        exclusions: editQuote.exclusions || "",
      ...
      setPaymentTerms(editQuote.paymentTerms || "");
      setDeliveryTerms(editQuote.deliveryTerms || "");
      setWarranty(editQuote.warranty || "");
  ...
  useEffect(() => {
  if (!editQuote && user && isOpen && !quoteData.id) {
    const userAny = user as any;
    
    // Only set if fields are currently empty
    if (!paymentTerms) setPaymentTerms(userAny.standardPaymentTerms || '');
    if (!deliveryTerms) setDeliveryTerms(userAny.standardDeliveryTerms || '');
    if (!warranty) setWarranty(userAny.standardWarranty || '');
    if (!exclusions) setExclusions(userAny.standardExclusions || '');
  }
}, [editQuote, user, isOpen]);
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
                <div>
                    ...
                    <textarea value={paymentTerms} onChange={(e) => setPaymentTerms(e.target.value)} rows={3} className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm" placeholder="e.g., 50% deposit, 50% on completion"></textarea>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Delivery Terms</label>
                    <textarea value={deliveryTerms} onChange={(e) => setDeliveryTerms(e.target.value)} rows={3} className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm" placeholder="e.g., Free delivery within 50 miles"></textarea>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Warranty</label>
                    <textarea value={warranty} onChange={(e) => setWarranty(e.target.value)} rows={3} className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm" placeholder="e.g., 12 months parts and labor"></textarea>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Exclusions</label>
                    <textarea value={exclusions} onChange={(e) => setExclusions(e.target.value)} rows={3} className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm" placeholder="e.g., Installation not included"></textarea>
                </div>
            </div>
          </div>
